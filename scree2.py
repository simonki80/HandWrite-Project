...
class SimpleDrawApp(LandscapeScreen):
    ...
    def enviar_imagen_al_backend(self, image):
        try:
            ...
            if response.status_code == 200:
                ...
                Clock.schedule_once(lambda dt: self.show_server_error_popup(), 0)
                return None
    ...
    def show_server_error_popup(self):
        self.dialog = MDDialog(
            title="[b][color=#FF0000]Server Error[/color][/b]",
            ...
            buttons=[
                MDRaisedButton(
                    ...
                    on_release=lambda x: self.dismiss_and_return()
                ),
            ],
        )
        self.dialog.open()

    def dismiss_and_return(self):
        self.dialog.dismiss()
        self.manager.current = 'calculator_screen'
